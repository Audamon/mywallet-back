...
describe('GET /wallet', () => {
    test('return 401 when token is invalid', async () => {
        const result = await supertest(server).get('/wallet').set("Authorization", `Bearer `);
        expect(result.status).toEqual(401);
    });
    test('return 200 when token is valid', async () => {
        const result = await supertest(server).get('/wallet').set("Authorization", `Bearer ${token}`);
